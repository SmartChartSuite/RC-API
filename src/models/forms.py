...
def save_form_questionnaire(questionnaire_dict: dict):
    ...
    try:
        ...
        logger.info(f"Found Questionnaire with name {questionnaire_dict['name']} and version {questionnaire_dict['version']}")
...
def convert_jobpackage_csv_to_questionnaire(jobpackage_csv: str, cql_only: bool, smartchart: bool, commit: bool) -> dict:
    ...
    questionnaire = deepcopy(questionnaire_template)
    ...
    if commit:
        return save_form_questionnaire(questionnaire)
